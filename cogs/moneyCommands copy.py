...
from sheets.PJSheetsController import PJRow, PJSheetController
from sheets import Value, SKIP
...
class Money(commands.Cog):
    ...
    async def pay(
        ...
        try:
            pj_row: PJRow = sh.get_pj_row(user_id)
            if transfertarget is not None:
                target_id: int = transfertarget.id
                target_pj_row: PJRow = sh.get_pj_row(target_id)
            else:
